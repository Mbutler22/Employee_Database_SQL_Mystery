...
SELECT first_name, last_name, hire_date FROM employees WHERE hire_date BETWEEN '1986-01-01' AND
'1987-01-01'

--3. List the manager of each department with the following information: department number, department name,
--the manager's employee number, last name, and first name.
SELECT departments.dept_no, departments.last_name, dept_manager.emp_no, employees.first_name, employees.last_name,
FROM departments JOIN dept_manager ON departments.dept_no=dept_manager.dept_no
JOIN employees ON dept_manager.emp_no = employees.emp_no;

--4. List the departments of each employee with the following information: employees number, last name,
---first name, and departments name.
SELECT dept_emp.emp_no, employees.last_name, employees.first_name, departments.dept_name
FROM dept_emp JOIN employees ON dept_emp.emp_no = employees.emp_no
JOIN departments ON dept_emp.dept_no = departments.dept_no;

--5. List first name, last name, and sex for employees whose first name is "Hercules" and last names begins
--with "B".
SELECT first_name, last_name
FROM employees WHERE first_name = "Hercules" AND last_name LIKE 'B%';

--6. List all employees in the Sales department, including thier employee number, last name, and department name.
SELECT dept_emp.emp_no, employees.last_name, employees.first_name, departments.dept_name
FROM dept_emp JOIN employees ON dept_emp.dept_no = employees.emp_no
JOIN departments ON dept_emp.dept_no = departments.dept_no WHERE departments.dept_name = 'Sales';

--7. List all employees in the Sales and Development departments, including thier employee number, last name,
